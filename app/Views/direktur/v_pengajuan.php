...
<head>
  ...
  <title>Data Pengajuan - Direktur</title>
    ...
    <div class="sidebar">
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
          ...
          <li class="nav-item">
            <a href="<?= base_url('direktur/pengajuan') ?>" class="nav-link active">
                ...
                <div class="card-tools">
                  <div class="input-group input-group-sm" style="width: 200px;">
                    <select id="filterStatus" class="form-control float-right">
                      ...
                      <option value="4">Revisi ke Accounting</option>
                    </select>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          elseif ($status == 4): 
                              ...
                              $status_text = 'Revisi ke Accounting';
                          else: 
                        ...
                        <td class="text-center">
                          <?php if ($status == 2): ?>
                            <!-- Tombol Proses hanya untuk status = 2 (Menunggu Approval Direktur) -->
                            <button class="btn btn-success btn-sm btn-proses" 
                                    data-id="<?= $row['id'] ?>"
                                    data-kode_barang="<?= $row['kode_barang'] ?>"
                                    data-nama_barang="<?= $row['nama_barang'] ?>"
                                    data-harga_barang="<?= $row['harga_barang'] ?? 0 ?>"
...
<!-- Modal Proses Pengajuan (untuk Direktur) - HANYA UNTUK PERSETUJUAN -->
<div class="modal fade" id="modalProses" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title"><i class="fas fa-check-circle"></i> Persetujuan Pengajuan - Direktur</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <?php echo form_open('direktur/proses_pengajuan'); ?>
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-info">
                            ...
                            <table width="100%">
                                <tr><td width="35%">Kode Barang</td><td>: <span id="proses_kode_barang">-</span></td></tr>
                                ...
                                <tr><td>Harga Pengajuan</td><td>: <span id="proses_harga_pengajuan">-</span></td></tr>
                ...
                <div class="form-group">
                    <label>Catatan Awal (dari pengaju)</label>
                    <textarea id="proses_catatan_awal" class="form-control" rows="3" readonly style="background-color: #f5f5f5;"></textarea>
                </div>

                <div class="form-group">
                    <label>Catatan Persetujuan Direktur <span class="text-danger">*</span></label>
                    <textarea name="catatan_direktur" id="proses_catatan_direktur" class="form-control" rows="4" placeholder="Masukkan catatan persetujuan dari Direktur..." required></textarea>
                    <small class="text-muted">Catatan ini akan menjadi catatan final setelah pengajuan disetujui</small>
                </div>
            </div>
            <div class="modal-footer">
                <!-- Tombol Tolak (sebelah kiri) - TIDAK ADA FORM, hanya trigger modal lain -->
                <button type="button" class="btn btn-danger" id="btnTolakPengajuan">
                    <i class="fas fa-times-circle"></i> Penolakan Pengajuan
                </button>
                <!-- <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button> -->
                <button type="submit" class="btn btn-success" id="btnSetujuPengajuan">
                    <i class="fas fa-check-circle"></i> Selesaikan Pengajuan
                </button>
            </div>
            <?php echo form_close(); ?>
        </div>
...
<div class="modal fade" id="modalKonfirmasiTolak" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open('direktur/tolak_pengajuan'); ?>
            <div class="modal-body">
                <input type="hidden" name="id_pengajuan" id="tolak_id_pengajuan">
                
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle"></i> 
                    <strong>Perhatian!</strong> Penolakan ini akan mengembalikan pengajuan ke Accounting untuk revisi.
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-info">
                            <strong>Detail Barang:</strong><br>
                            <table width="100%">
                                <tr><td width="35%">Kode Barang</td><td>: <span id="tolak_kode_barang">-</span></td></tr>
                                <tr><td>Nama Barang</td><td>: <span id="tolak_nama_barang">-</span></td></tr>
                                <tr><td>Harga Barang</td><td>: <span id="tolak_harga_barang">-</span></td></tr>
                                <tr><td>Harga Pengajuan</td><td>: <span id="tolak_harga_pengajuan">-</span></td></tr>
                                <tr><td>Kategori</td><td>: <span id="tolak_kategori">-</span></td></tr>
                                <tr><td>Lokasi</td><td>: <span id="tolak_lokasi">-</span></td></tr>
                                <tr><td>Tanggal Pengajuan</td><td>: <span id="tolak_tgl_pengajuan">-</span></td></tr>
                            </table>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="catatan_direktur" id="tolak_catatan_direktur" class="form-control" rows="4" placeholder="Masukkan alasan penolakan dan catatan revisi..." required></textarea>
                    <small class="text-muted">Catatan ini akan dikirim ke Accounting untuk revisi</small>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
...
<script>
  $(function () {
    ...
    // Filter status (tambahkan status 4)
    $('#filterStatus').on('change', function() {
        ...
        if (status === "") {
            ...
            table.column(7).search('Revisi ke Accounting').draw();
        }
    });
    ...
    $('.btn-proses').on('click', function() {
        ...
        // Format harga pengajuan
        var hargaPengajuan = button.data('harga_pengajuan') || 0;
        $('#proses_harga_pengajuan').text('Rp ' + new Intl.NumberFormat('id-ID').format(hargaPengajuan));
        ...
        $('#proses_status').html('<span class="badge bg-info">Menunggu Approval Direktur</span>');
        $('#proses_catatan_awal').val(button.data('catatan_awal') || '-');
        $('#proses_catatan_direktur').val('');
        
        // Simpan data untuk tombol tolak
        $('#btnTolakPengajuan').data('id', button.data('id'));
        $('#btnTolakPengajuan').data('kode_barang', button.data('kode_barang'));
        $('#btnTolakPengajuan').data('nama_barang', button.data('nama_barang'));
        $('#btnTolakPengajuan').data('harga_barang', button.data('harga_barang'));
        $('#btnTolakPengajuan').data('harga_pengajuan', button.data('harga_pengajuan'));
        $('#btnTolakPengajuan').data('kategori', button.data('kategori'));
        $('#btnTolakPengajuan').data('lokasi', button.data('lokasi'));
        $('#btnTolakPengajuan').data('tgl_pengajuan', button.data('tgl_pengajuan'));
        
        $('#modalProses').modal('show');
    });

    // Tombol Penolakan (buka modal konfirmasi)
    $('#btnTolakPengajuan').on('click', function() {
        var button = $(this);
        
        // Tutup modal proses
        $('#modalProses').modal('hide');
        
        // Siapkan data untuk modal konfirmasi tolak
        $('#tolak_id_pengajuan').val(button.data('id'));
        $('#tolak_kode_barang').text(button.data('kode_barang') || '-');
        $('#tolak_nama_barang').text(button.data('nama_barang') || '-');
        
        var hargaBarang = button.data('harga_barang') || 0;
        $('#tolak_harga_barang').text('Rp ' + new Intl.NumberFormat('id-ID').format(hargaBarang));
        
        var hargaPengajuan = button.data('harga_pengajuan') || 0;
        $('#tolak_harga_pengajuan').text('Rp ' + new Intl.NumberFormat('id-ID').format(hargaPengajuan));
        
        $('#tolak_kategori').text(button.data('kategori') || '-');
        $('#tolak_lokasi').text(button.data('lokasi') || '-');
        $('#tolak_tgl_pengajuan').text(button.data('tgl_pengajuan') || '-');
        $('#tolak_catatan_direktur').val('');
